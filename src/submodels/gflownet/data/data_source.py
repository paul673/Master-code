...
from src.submodels.gflownet.data.replay_buffer import ReplayBuffer, detach_and_cpu
from src.submodels.gflownet.envs.graph_building_env import GraphBuildingEnvContext
from src.submodels.gflownet.utils.misc import get_worker_rng
...
class DataSource(IterableDataset):
    ...
    def compute_properties(self, trajs, mark_as_online=False):
        ...
        #valid_idcs = torch.tensor([i for i in range(len(trajs)) if trajs[i].get("is_valid", False)]).long() #Invalidate molecules per default to fix invalid mask
        ...
        # Paul Addition TODO Ensure that not all is_valid values are set to true
